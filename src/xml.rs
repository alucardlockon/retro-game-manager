...
pub fn parse_games_from_file(path: &Path) -> Result<Vec<GameEntry>> {
	let platform = infer_platform_from_filename(path)
		.unwrap_or_else(|| "Unknown".to_string());

	let mut reader = Reader::from_file(path)
		.with_context(|| format!("读取 XML 失败: {}", path.display()))?;
		...
		match reader.read_event_into(&mut buf) {
			Ok(Event::Start(e)) => {
				match e.name() {
					...
					n if n == QName(b"details") && in_game => {
...
pub fn extract_game_xml_by_index(path: &Path, target_idx: usize) -> Result<String> {
	...
	loop {
		match reader.read_event_into(&mut buf) {
			...
			Ok(Event::CData(e)) => {
				if capturing { writer.write_event(Event::CData(e)).ok(); }
			}
			Ok(Event::Comment(e)) => {
				if capturing { writer.write_event(Event::Comment(e)).ok(); }
			}
			Ok(Event::End(e)) => {
				if capturing {
